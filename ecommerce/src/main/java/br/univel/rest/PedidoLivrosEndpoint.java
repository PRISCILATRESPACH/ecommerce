...
@Stateless
@Path("/pedidolivros")
public class PedidoLivrosEndpoint
{
   ...
   public Response create(PedidoLivros entity)
   {
      em.persist(entity);
      return Response.created(UriBuilder.fromResource(PedidoLivrosEndpoint.class).path(String.valueOf(entity.getId())).build()).build();
   }
   ...
   public List<PedidoLivros> listAll(@QueryParam("start") Integer startPosition, @QueryParam("max") Integer maxResult)
   {
      ...
      final List<PedidoLivros> results = findAllQuery.getResultList();
      return results;
   }
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Consumes("application/json")
   public Response update(@PathParam("id") Long id, PedidoLivros entity)
   {
      if (entity == null)
      {
      ...
      if (em.find(PedidoLivros.class, id) == null)
      {
         return Response.status(Status.NOT_FOUND).build();
      }
      try
